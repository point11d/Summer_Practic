...
int menu(int);
void PorciyaNaCheloveka(struct shashlik*, int);
void ShashlSOdnimMyasom(struct shashlik*, int);
void Alfavit(struct shashlik*, int, struct spisokShashlik**);
void DolgiyShashlPoMyasu(struct shashlik*, char moloko[30], int);
void ShashlDoMassi(struct shashlik*, int cost, int);
void Diagramma(struct shashlik*, int, struct spisokShashlik**);
void SlozhniyVopros(struct shashlik*, int);
void vstavka(struct shashlik* , char* , int, struct spisokShashlik**);
...
int main()
{
	...
	for (i = 0; i < chisloStrok; i++)
		printf("\n%-30s %-20s %7d %7d %ld", mashlik[i].nazvanie, mashlik[i].myaso,
			mashlik[i].vremya, mashlik[i].kilo, mashlik[i].porcii);
	getch();
	struct spisokShashlik* spisok = 0;
	while (true)
	{
		...
		case 1: {
			...
			PorciyaNaCheloveka(mashlik, chisloStrok); break;
		}
		case 2: Alfavit(mashlik, chisloStrok, &spisok); break;
		case 3: {
			Console::CursorTop = 14;
			Console::CursorLeft = 11;
			char tmp[20];
			...
			scanf("%s", tmp);
			DolgiyShashlPoMyasu(mashlik, tmp, chisloStrok); break;
		}
		case 4: {
			Console::CursorTop = 14;
			Console::CursorLeft = 11;
			...
			scanf("%ld", &tmpInp);
			ShashlDoMassi(mashlik, tmpInp, chisloStrok); break;
		}
		case 5: SlozhniyVopros(mashlik, chisloStrok); break;
		case 6: Diagramma(mashlik, chisloStrok, &spisok); break;
		case 7: exit(0);
		}
	} // конец while(1)...
	return 0;
...
void PorciyaNaCheloveka(struct shashlik* mashlik, int chisloStrok) {
	Console::ForegroundColor = ConsoleColor::White;
	Console::BackgroundColor = ConsoleColor::DarkRed;
...
void vstavka(struct shashlik* client, char* name, int chisloStrok, struct spisokShashlik**spisok)
{
	int i;
...
void Alfavit(struct shashlik* client, int chisloStrok, struct spisokShashlik**spisok)
{
	int i;
	...
	if (!*spisok) {
		for (i = 0; i < chisloStrok; i++) {
			vstavka(client, client[i].nazvanie, chisloStrok, spisok);
		}
	}
	Console::Clear();
...
void DolgiyShashlPoMyasu(struct shashlik* mashlik, char myaso[30], int chisloStrok)
{
	Console::ForegroundColor = ConsoleColor::White;
	Console::BackgroundColor = ConsoleColor::DarkRed;
	int overall = 0;
...
void ShashlDoMassi(struct shashlik* clients, int kilo, int chisloStrok)
{
	Console::ForegroundColor = ConsoleColor::White;
	Console::BackgroundColor = ConsoleColor::DarkRed;
	Console::CursorTop = 16;
...
void Diagramma(struct shashlik* clients, int chisloStrok, struct spisokShashlik**spisok)
{
	struct spisokShashlik* nt;
	int len, i, NColor;
	long sum = 0;
	...
	if (!*spisok)
		for (i = 0; i < chisloStrok; i++)
			vstavka(clients, clients[i].nazvanie, chisloStrok, spisok);
...
void SlozhniyVopros(struct shashlik* mashlik, int chisloStrok) {
	bool check = false;
